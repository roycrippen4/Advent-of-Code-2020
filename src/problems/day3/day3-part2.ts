...
export function solveD3P2() {
  const tile = getData(filepath);
  const numberOfTrees: number[] = [];
  for (const slope of slopes) {
    const map = buildMap(slope.x, tile);
    numberOfTrees.push(countTrees(slope.x, slope.y, map));
  }
  const answer = numberOfTrees.reduce((a, b) => a * b, 1);
  console.log(answer);
  return answer;
}
...
function buildMap(xVel: number, data: number[][]): number[][] {
  const numberOfTiles = calculateTileAmount(xVel, data);
  return addTiles(numberOfTiles, data);
}

function countTrees(xVel: number, yVel: number, map: number[][]): number {
  let count = 0;
  let y = 0;
  let x = 0;
  while (y < map.length) {
