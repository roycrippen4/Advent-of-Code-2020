...
export function solveD5P2() {
  const passIDs = collectIDs(getData(filepath));
  const answer = getBoardingPass(passIDs);
  console.log(answer);
  return answer;
}

function getBoardingPass(passIDs: number[]) {
  for (let i = 1; i < passIDs.length - 1; i++) {
    if (passIDs[i] + 1 !== passIDs[i + 1] && passIDs[i + 1] !== passIDs[i] - 1) {
      return passIDs[i] + 1;
    }
  }
  assertUnreachable();
...
function collectIDs(boardingPasses: string[]): number[] {
  const IDs: number[] = [];
  for (const boardingPass of boardingPasses) {
    IDs.push(getID(boardingPass));
  }
  return IDs.sort((x, y) => {
